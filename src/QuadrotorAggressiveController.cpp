...
QuadrotorAggressiveController::QuadrotorAggressiveController(struct DataCentre* wrap_data_ptr, struct NNFeedforwardCmd* ff_cmd_ptr){
...
void QuadrotorAggressiveController::loadFeedforwardData(){
  ...
  ff_cmd.position_cmd  = ff_ptr->position_cmd;
  ff_cmd.velocity_cmd = ff_ptr->velocity_cmd;
  ff_cmd.attitude_cmd = ff_ptr->attitude_cmd;
  ff_cmd.rate_cmd = ff_ptr->rate_cmd;
  ff_cmd.thrust = ff_ptr->thrust;

  if (ff_ptr->lambda0 < 0.01 & ff_ptr->lambda1 < 0.1)
    ff_cmd.is_done = 1;
  else
    ff_cmd.is_done = 0;  
...
void QuadrotorAggressiveController::aggressiveControl(){
  ...
  Eigen::Matrix3d k_position, k_velocity;
  ...
  if(psi_des > 0.7) psi_des = 0.7;
  if(psi_des < -0.7) psi_des = -0.7;
  if(phi_des > 0.7) phi_des = 0.7;
  if(phi_des < -0.7) phi_des = -0.7;
  if(theta_des > 0.7) theta_des = 0.7;
  if(theta_des < -0.7) theta_des = -0.7;
  if(thrust_des > 0.95) thrust_des = 0.95;
  if(thrust_des < 0) thrust_des = 0;

  Eigen::Vector3d attitude_cmd(phi_des,theta_des,psi_des);
  cout << "psi_des:" << psi_des << endl;
  Eigen::Vector3d rate_fb = kp_attitude_ * (attitude_cmd - current_attitude_);
